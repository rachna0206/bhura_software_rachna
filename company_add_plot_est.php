  ...
  try {
    ...
    $stmt = $obj->con1->prepare("
          SELECT 
              ...
              (SELECT stage FROM tbl_tdrawassign WHERE inq_id = r1.id ORDER BY id DESC LIMIT 1) AS stage, 
              (SELECT 
                  CASE 
                      WHEN stage = 'lead' THEN 'Positive' 
                      WHEN stage = 'badlead' THEN 'Negative' 
                      ELSE 'Existing Client' 
                  END 
              FROM tbl_tdrawassign WHERE inq_id = r1.id ORDER BY id DESC LIMIT 1) AS stage1 
          FROM tbl_tdrawdata r1 
          WHERE 
              r1.raw_data->'$.post_fields.Taluka' = ? 
      ...
      <tbody class="table-border-bottom-0">
        ...
        while ($data = mysqli_fetch_array($result)) {
          ...
          <tr>
            ...
            <td>
              <a
                href="javascript:viewdata('<?php echo base64_encode($data["id"]) ?>','<?php echo base64_encode($data["state_id"]) ?>','<?php echo base64_encode($data["city_id"]) ?>','<?php echo base64_encode($data["taluka"]) ?>','<?php echo base64_encode($data["area_id"]) ?>','<?php echo base64_encode($data["industrial_estate"]) ?>','<?php echo base64_encode($user_id) ?>');">View</a>

              <a
                href="company_add_plot_est.php?download_single=true&taluka=<?php echo $data['taluka'] ?>&area_id=<?php echo $data['area_id'] ?>&industrial_estate=<?php echo $data['industrial_estate'] ?>"><i
