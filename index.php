...
if(isset($_REQUEST["login"])){
      ...
      echo "</br>".$plaintext_password = $_REQUEST["password"];
     
      $qr = $obj->con1->prepare("select * from tbl_users where email=?");
      $qr->bind_param("s",$ui);
      $qr->execute();
      $result = $qr->get_result();
      $num=mysqli_num_rows($result);
      $row=mysqli_fetch_array($result);
      echo "</br>".$hashed_pass=$row["password"];
      $qr->close();

      $verify = password_verify($plaintext_password, $hashed_pass);
      echo $verify;
