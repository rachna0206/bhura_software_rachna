...
if(isset($_REQUEST['btn_modal_update']))
{
  ...
  try
  {
    ...
    $stmt_del = $obj->con1->prepare("DELETE from assign_estate where industrial_estate_id=? and action='estate_plotting'");
  ...
  if($Resp)
  {
    ...
    header("location:assign_estate_plotting.php");
  }
  else
  {
...
<h4 class="fw-bold py-3 mb-4">Assigned Estate Master
<span class="text-right"><a href="map.php">View Map</a></span>
</h4>
      ...
      <div class="table-responsive text-nowrap">
        <table class="table table-hover" id="table_id">
          ...
          <tbody class="table-border-bottom-0">
            <?php 
              // assigned estates for plotting (not shown those whose plotting is done)
              $stmt_list = $obj->con1->prepare("SELECT a1.*, u1.name, i1.state_id, i1.city_id, i1.taluka, i1.area_id, i1.industrial_estate from assign_estate a1, tbl_users u1, tbl_industrial_estate i1 where a1.employee_id=u1.id and a1.industrial_estate_id=i1.id and action='estate_plotting' and a1.industrial_estate_id not in (SELECT industrial_estate_id FROM `pr_add_industrialestate_details` where plotting_pattern is not null) and a1.industrial_estate_id not in (SELECT industrial_estate_id FROM `pr_add_industrialestate_details` where status!='Verified') order by a1.id desc");
              $stmt_list->execute();
...
<script type="text/javascript">
  ...
  function deletedata(id,img) {
    if(confirm("Are you sure to DELETE data?")) {
        var loc = "assign_estate_plotting.php?flg=del&n_id="+id;
        window.location = loc;
    }
  }
  ...
  function editdata(id,emp_id,state,city,taluka,area,ind_estate,ind_estate_id,start_dt,end_dt) {
    ...
    $.ajax({
      ...
      data: "id="+id+"&ind_estate_id="+ind_estate_id+"&pg=estate_plotting",
      cache: false,
  ...
  function plottingGrid(name,city_id,taluka,area_id,industrial_estate,start_dt,end_dt){
    ...
    window.open('assigned_estate_plotting_excel.php', '_blank');
