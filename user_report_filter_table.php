...
if ($_REQUEST["start_date"] != "") {
    ...
    array_push($captions, "Start Date: " . date("d-m-Y", strtotime($_REQUEST["start_date"])));
}
...
if ($_REQUEST["end_date"] != "") {
    ...
    array_push($captions, "End Date: " . date("d-m-Y", strtotime($_REQUEST["start_date"])));
}
...
if ($_REQUEST["select_user_id"] != "") {
    ...
    $name_stmt = $obj->con1->prepare("SELECT name FROM tbl_users WHERE id = ?");
    $name_stmt->bind_param("s", $_REQUEST["select_user_id"]);
    $name_stmt->execute();
    $result = $name_stmt->get_result();
    $result = $result->fetch_assoc()["name"];
    $name_stmt->close();
    array_push($captions, "Username: " . $result);
}
...
<caption class="caption-top fw-bold fst-italic text-dark" id="table_caption"><?= implode(", ", $captions) ?></caption>
