...
class DbOperation
{
    ...
    public function add_industrial_estate($state, $city, $taluka, $area, $industrial_estate, $description, $plotting_pattern, $location, $userid, $verify_status)
    {
        ...
        if ($Resp) {
            ...
            $comp = "";
            $comppany_id = 0;
            $this->add_user_activity($userid, $industrial_estate, $comp, $operation, $insert_id, $comppany_id);
    ...
    public function insert_followup($user_id, $id, $followup_text, $followup_source, $followup_date, $operation = "Follow up added")
    {
        ...
        $stmt_rawdata = $this->con->prepare("select tr.raw_data,pr.cid as company_id,pr.industrial_estate_id from tbl_tdrawdata tr,pr_company_details pr where pr.rawdata_id=tr.id and  tr.id=?");
        $stmt_rawdata->bind_param("i", $id);
        $stmt_rawdata->execute();
        $resp_rawdata = $stmt_rawdata->get_result()->fetch_assoc();
        $stmt_rawdata->close();
        $data = json_decode($resp_rawdata["raw_data"]);
        //print_r($data);
        $industrial_estate = $data->post_fields->IndustrialEstate;
        $company = $data->post_fields->Firm_Name;
        $industrial_estate_id = $resp_rawdata["industrial_estate_id"];
        $comppany_id = $resp_rawdata["company_id"];
        ...
        $resp_activity = $this->add_user_activity($user_id, $industrial_estate, $company, $operation, $industrial_estate_id, $comppany_id);
    ...
    public function insert_pr_company_detail($source, $source_name, $contact_person, $contact_no, $firm_name, $gst_no, $category, $segment, $premise, $state, $city, $taluka, $area, $industrial_estate, $remark, $inq_submit, $PicFileName, $constitution, $status, $industrial_estate_id, $user_id, $id, $plot_status, $pr_company_plot_id, $pr_company_detail_id, $location, $existing_expansion_status, $update_location)
    {
        if ($pr_company_detail_id == "" || $pr_company_detail_id == null || $pr_company_detail_id == "null") {
            ...
            $this->add_user_activity($user_id, $industrial_estate, $firm_name, $operation, $industrial_estate_id, $last_insert_company_id);
            ...
            if ($num_rows_aff > 0) {
                $this->add_user_activity($user_id, $industrial_estate, $firm_name, $operation, $industrial_estate_id, $pr_company_detail_id);
            }
...
WHERE 
    ...
    //add user acivity
    private function add_user_activity($userid, $industrial_estate, $company, $operation, $industrial_estate_id, $company_id)
    {
        $stmt_activity = $this->con->prepare("INSERT INTO `pr_user_activity`( `industrial_estate`, `company`, `user_id`,`industrial_estate_id`,`company_id`, `operation`) VALUES (?,?,?,?,?,?)");
        $stmt_activity->bind_param("sssiis", $industrial_estate, $company, $userid, $industrial_estate_id, $company_id, $operation);
